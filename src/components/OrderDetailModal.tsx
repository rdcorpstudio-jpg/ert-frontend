...
export default function OrderDetailModal({
  ...
  const handleAddOrderFreebie = async () => {
    ...
    try {
      await api.post(`/orders/${detail.order.id}/freebies`, null, {
  ...
  const handleAddItemFreebie = async () => {
    ...
    const freebie = availableFreebies.find((f) => f.id === addingItemFreebieId);
    if (!freebie) return;
    try {
      await api.post(`/orders/items/${mainItem.id}/freebies`, null, {
        params: { freebie_name: freebie.name },
      });
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={box} onClick={(e) => e.stopPropagation()}>
        ...
        {productEditable && availableFreebies.length > 0 && (
